...
> PATCH can be idempotent if implemented in that way

## Idempotent Keys

These are keys generated by client or server to make the non idempotent methods idempotent in implementation. This solves the issues on retry of these requests.

Server saves this idempotent key with the data and compare it if the same request comes again, for modification of data.

Example: etag in Cosmos
...
| ↳ 301   | Moved Temporarily     | Resource redirected to a new URL   |
...
**Offset Based Pagination** - Provide offset and count.

- But in large data, if data changes in between, can result in missed or repeated data

```
/events?offset=20&limit=10
```

**Cursor-based Pagination** - Provide pointer - Encoded reference to specific record / timestamp
...
Filter and sort the lists via query params
